...
import logging
import json
from .chat_ai_agent import process_ai_message
    ...
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            UserProfile.objects.create(
                owner=user, nick_name=nick_name, gender=gender, birthday=birthday
            )
...
@require_POST
@csrf_exempt  # 开发阶段可以先豁免CSRF，生产环境请使用正确的方式
def chat_api(request):
    ...
    try:
        ...
        logger.info(f"收到消息: {user_message}, 会话ID: {session_id}")
